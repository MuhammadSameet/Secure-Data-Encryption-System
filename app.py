...
# user registration
    ...
    st.subheader("Register Your User")
    username = st.text_input("Choose Username")
    password = st.text_input("choice password", type="password")

    if st.button("Register"):
        if username and password:
            if username in store_data:
                st.warning("⚠️User Already Register")
            else:
                store_data[username] = {
            ...
            st.error("Both Fileds are Required")
elif choice == "Login":
        st.subheader("User Logon")

        if time.time() < st.session_state.lockout_time:
            remainig = int(st.session_state.lockout_time - time.time())
            st.error(f"⏱️ Too many failed attempts. please wait {remainig} seconds.")
            st.stop()
        ...
        if st.button("Login"):
            if username in store_data and store_data[username]["password"] == hash_password(password):
                ...
                if st.session_state.falied_attempts >= 3:
                    st.session_state.lockout_time = time.time() + LOCKOUT_DURATION
                    st.error("🛑 To many failed attemps. Locked for 60 second")
                    st.stop()
...
# data score 
    ...
    if not st.session_state.authenticated_user:
        ...
        passkey = st.text_input("Encryption key (passphases)", type="password")
